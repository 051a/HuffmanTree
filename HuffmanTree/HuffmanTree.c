 ...
 *   权值相同时，原序列中先出现的作为左孩子
 *
 * VS:     项目属性 → C/C++ → 命令行 → 其他选项 → 添加 /utf-8
 * Dev-C++: 工具 → 编译选项 → 编译时加入: -finput-charset=utf-8 -fexec-charset=utf-8
 */
...
#ifdef _MSC_VER
#pragma execution_character_set("utf-8")  /* MSVC专用: 字符串以UTF-8存储 */
#endif

#ifdef _WIN32
#include <windows.h>                      /* SetConsoleOutputCP (VS和Dev-C++均可用) */
#endif
